...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(401).json({ error: 'Invalid credentials' }); //this is if the user doesnt exist, as if we query a non existing user the password would be null, or if the
    }
